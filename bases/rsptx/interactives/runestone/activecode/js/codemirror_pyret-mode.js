/* codemirror_pyret-mode.js */
//
// CodeMirror mode for Pyret. This is a thin export wrapper around the actual
// tokenizer from the DCIC textbook's pyret-hilite/pyret.js (dcic-source
// repo), which already self-registers via a bare top-level
// "CodeMirror.defineMode(...)" call assuming a global CodeMirror. Runestone
// Components uses ES modules with no global CodeMirror, so the whole mode
// body below is wrapped in this exported function and the CodeMirror
// parameter is threaded through instead, following the same
// registerXXXMode(CodeMirror) pattern as codemirror_gdscript-mode.js.
//
// Deliberately NOT carried over from pyret-hilite: the "fold: \"pyret\""
// and "indent" properties on the mode object reference a separate
// CodeMirror.registerHelper("fold", "pyret", ...) addon that DCIC never
// ships in pyret-hilite/ either -- those properties are left in place
// below (harmless no-ops without the helper registered) but no fold/indent
// addon is wired up here. The editor will tokenize/highlight correctly; it
// just won't get Pyret-aware code folding or smart-indent-on-Enter.
export function registerPyretMode(CodeMirror) {
CodeMirror.defineMode("pyret", function(config, parserConfig) {
  ...
  function parse(firstTokenInLine, state, stream, style) {
    // NOTE: pyret-hilite/pyret.js (dcic-source) has this as a bare
    // "ls = state.lineState;" with no declaration, which only worked because
    // the original file is loaded as a classic sloppy-mode <script>, where an
    // undeclared assignment silently creates a global. Wrapped here as a
    // strict-mode ES module, that throws a ReferenceError instead, so it's
    // declared properly as a local.
